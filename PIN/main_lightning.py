...
from model_lightning.pin_lightning_with_test import LitPinModule, myCallBack
...
def get_args():
    ...
    parser.add_argument("--multi_channel_dims", default='[1,2]')
    parser.add_argument("--mid_dim", default='h')
...
def main():
    ...
    train_loader, val_loader, test_loader = make_daataset(norm_df, args.batch_size, args.windows_size, pred_length=10, split=str2int_list(args.split))

    mid_dim = int(feature_size/2) if args.mid_dim == 'h' else int(args.mid_dim)
    ...
    devices_index_list = str2int_list(args.devices)
    ...
    model = LitPinModule(writer, max_steps, dataset_name, [args.windows_size, feature_size], mid_dim, 
